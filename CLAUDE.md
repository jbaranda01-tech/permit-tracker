...
**Idempotent bulk import (no duplicates):** `import_issues()` (`issues/routes.py`) is safe to re-run on the same file. Each row's identity is `(equipment_id, category, normalized description, reported date)` — severity/status are excluded so corrections re-apply instead of duplicating. Before inserting, it checks both an in-batch `seen` map (duplicate rows in the same file) and the DB via the `find_duplicate_issue()` helper (rows from a prior import). On a match it does NOT create a new `Issue`; it updates the existing one's severity and (only if the status changed) re-runs `synthesize_status_history`. The results page reports imported / duplicates-skipped / errored counts. Driver reports are never a duplicate source (one at a time, live timestamps). `flask dedup` cleans up any pre-existing import duplicates by the same key (keeps lowest id; child `IssueStatusHistory` cascade-deletes). No hard DB unique constraint exists for issues — a legitimate same-day re-report of the same fault is valid data, so enforcement is deliberately at the import layer only.
...
- `static/css/style.css` — All CSS in one file (~1830 lines). Custom CSS with CSS variables, no framework. Three responsive breakpoints (1024px, 768px, 480px). Dual theme (light/dark) via `data-theme` attribute.
