...
- **No future-dated issue reports:** `_is_future_date()` (`issues/routes.py`) guards every path that sets an issue's `reported_at`/`resolved_at` so a date after today (server date) is rejected — the manual-entry route `create()` adds a Spanish error and re-renders with HTTP 422; the bulk import counts the row as errored and `continue`s (nothing persists). This exists because `_coerce_date`'s 2-digit-year `+100` bump (correct for e.g. `25`→2025) could turn a spreadsheet `1935`/`35` into `2035`; the guard catches the pathological result at point-of-use rather than removing the bump other importers rely on. The **driver report flow never sets a date** (`report.html` has no date field; `report_submit` relies on the `Issue.reported_at` default `datetime.utcnow`) — do NOT add a date field there.
